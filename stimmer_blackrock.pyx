...
pulse_width = 1 # In ms - original code 0.09 # duration of pulse (ms) # duration of pulse (ms) .9
width = pulse_width * 1.23287671233 # This corrects for expected vs. actual output
...
biphasic = False # Changed 2/3/23 by Jon W. to get to work with BlackRock System
phaseShift = 0
'''
...
def waitForEvent(stimX, stimY, q, backQ, channel, microamps, stimLag, timer, timeout, timeoutVar, oeAddress):
  ...
  window = Tk()
  winText = "Change to stim location for Closed Loop"
  ...
  while True:
    ...
    if jsonStr:
      if curTime - stimTime > curTimeout: # wait timeout
        if jsonStr['type'] == 'ttl' and int(jsonStr['channel']) == int(channel)-1 and jsonStr['data'] == True: # ttl and data==true! and only phase crossing detector channel 
          if stimSent == 0: # last was sham, send stim now
            if stimLag > 0:
                time.sleep(stimLag)
            stimX.sendWaveform(npWave)
            #q.put('Closed loop pulse sent,' + stimX.address)
            print('Closed loop pulse sent,' + stimX.address)
            stimTime = time.perf_counter()
            stimSent = 1
            curTimeout = random.uniform(timeoutHigh, timeoutLow)
          else:
            ...
            print('Closed loop sham pulse sent,' + stimY.address)
            stimTime = time.perf_counter()
            stimSent = 0
            curTimeout = random.uniform(timeoutHigh, timeoutLow) # sham timeout. Comment out to remove

def ERP(stimX, stimY, q, backQ, nERP, ERP_INTER_LOW, ERP_INTER_HIGH, NUM_LOCATIONS, snd, suffix, amplitude = 1):
    '''
    ...
    while not backQ.empty():
        ...
        print(msg)
    # Custom ERP Settings
    npWave = createWaveform(amplitude)
...
def ERPTitration(stimX, nERP, startuA, enduA, stepSize):
  startV = float(startuA) / 100.0
  endV = float(enduA) / 100.0
  stepV = float(stepSize) / 100.0
  voltsList = np.arange(startV, endV, stepV)

  for i in range(len(voltsList)):
    curV = voltsList[i]
    npWave = createWaveform(curV)
    for n in range(nERP):
      stimX.sendWaveform(npWave)
      print('titration sent, ' + str(curV*100) + 'uA')
      # Wait for brain to return to normal before stimming again
      sleepLen = random.uniform(3, 5)
      time.sleep(sleepLen) # 4 +- 1 second
